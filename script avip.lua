...
local fuk = Tabs.player:AddLeftGroupbox('Hood Custom')
...
fuk:AddToggle('sigma', {
    ...
    Callback = function(isEnabled)
        ...
        if running == 0 then
            local fireRateEnabled = isEnabled;

            while fireRateEnabled do
                local player = game:GetService("Players").LocalPlayer;

                -- Duyệt qua các đối tượng trong Backpack
                for _, item in pairs(player.Backpack:GetChildren()) do
                    if item:FindFirstChild("GunData") then
                        local gunData = item:FindFirstChild("GunData");

                        if gunData and gunData:IsA("ModuleScript") then
                            local gunSettings = require(gunData);

                            -- Nếu thuộc tính cooldown và slowdown_time tồn tại, chỉnh sửa chúng
                            if gunSettings.cooldown and gunSettings.slowdown_time then
                                gunSettings.cooldown = gunCooldown;
                                gunSettings.slowdown_time = gunCooldown;
                            end
                        end
                    end
                end

                task.wait(2); -- Chờ 2 giây trước khi lặp lại
            end
        end



fuk:AddInput('wantfuckrn', {
    Default = tostring(gunCooldown),
    Numeric = true,
    Finished = false,   
    Text = 'Gun Delay',
    Tooltip = nil, 
    ...
    Callback = function(newCooldown)
        gunCooldown = tonumber(newCooldown)
    end
})
